...
namespace Basic.HrDAL.Repository
{
    internal class BasicDAL<T, IdentityId> : IBasicDAL<T, IdentityId> where T : class, new()
    {
        ...
        {
            Result res = this._BasicDAL.GetById<IdentityId, Result>(id, selector);
            if (res == null)
            {
                throw new ErrorException("hr." + this._prefix + ".not.find");
            }
        ...
        {
            T[] result = this._BasicDAL.GetsById<IdentityId>(ids);
            if (result.IsNull())
            {
                throw new ErrorException("hr." + this._prefix + ".not.find");
            }
        ...
        {
            Result[] result = this._BasicDAL.GetsById<IdentityId, Result>(ids, selector);
            if (result.IsNull())
            {
                throw new ErrorException("hr." + this._prefix + ".not.find");
            }
        ...
        {
            return this._BasicDAL.Update<Set>(sourc, set);
        }
    }
}
