...
export default {
  ...
  methods: {
    reset() {
      ...
      if (this.value.config && this.value.config.RollAdvert) {
        this.item.cover = this.exampleImg[this.value.config.RollAdvert.PlaceId]
      }
      if (this.item.cover == null) {
        this.item.cover = '/pageTemplate/image/menu.png'
      }
    },
    chiose() {
